...
stat.清洁.add = new Function(`
  ...
  if(stat.清洁.current>stat.心情.preset){stat.清洁.current=stat.清洁.preset;}
  localStorage.setItem("status_清洁值",stat.清洁.current);
  petmenu_close();
`);
...
stat.心情.refresh = new Function(`
  if(stat.心情.current<(stat.心情.preset/4)&&pet.imgs.frown){petload(pet.imgs.frown);return;}
  if(stat.心情.current<(stat.心情.preset/2)&&pet.imgs.calm){petload(pet.imgs.calm);return;}
`)
